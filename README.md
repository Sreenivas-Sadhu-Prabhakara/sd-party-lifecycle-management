...
| POST | `/v1/party-record-management-plan/initiate` | Initiate |
| GET | `/v1/party-record-management-plan` | Retrieve (list) |
| GET | `/v1/party-record-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/party-record-management-plan/{crId}/update` | Update |
| PUT | `/v1/party-record-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/party-record-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-party-lifecycle-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-party-lifecycle-management ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-party-lifecycle-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
